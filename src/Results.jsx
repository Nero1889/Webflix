...
function Results() {
    ...
    return (
        <div className="bg-slate-950 min-h-screen text-white">
            <div className="flex items-start mt-[.5rem] p-5 sticky top-0 bg-slate-950 sm:mx-[1.5rem] xl:mx-[3.5rem] 2xl:mx-[5.5rem]">
                ...
                <div className="relative flex-grow flex">
                    ...
                    <input
                        type="text"
                        placeholder="Search!"
                        className="w-full h-[2.7rem] bg-slate-900 text-white pl-[3rem] rounded-[7rem] focus:outline-none text-sm"
            ...
            {loading && (
                <div className="flex justify-center items-center mt-20">
                    <div className="w-[2.5rem] h-[2.5rem] border-4 border-slate-800 border-t-slate-300 rounded-full animate-spin"></div>
                </div>
            )}
            {error && (
                <p className="text-[#b71234] text-center mt-4 p-4 border-rose-600 border-[2px] mx-5 sm:mx-[3rem] xl:mx-[5rem] 2xl:mx-[7rem] rounded-md">
                    {error}
                </p>
            )}
            {searchResults.length > 0 && (
                <>
                    <h1 className="text-slate-500 text-sm font-[650] mb-[1rem] ml-[2rem] sm:ml-[3rem] sm:text-base lg:text-lg xl:ml-[5rem] 2xl:ml-[7rem]">
                        Search Results For: <span className="text-slate-300 ml-[.35rem]">{searchQuery}</span>
                    </h1>
                    <div className="grid grid-cols-2 mx-[2rem] sm:grid-cols-3 sm:mx-[3rem] md:grid-cols-4 lg:grid-cols-5 2xl:grid-cols-6 gap-4 xl:mx-[5rem] 2xl:mx-[7rem] pb-8">
                        {searchResults.map((item) => (
                            <div key={item.id} onClick={() => handleItemClick(item)}
                                className="p-3 bg-slate-900 rounded-[1rem] flex flex-col items-center hover:bg-slate-800 transition-colors duration-[.25s] cursor-pointer">
                                {item.media_type === "person" ? (
                                    item.profile_path ? (
                                        <img src={`${TMDB_IMAGE_BASE_URL}${item.profile_path}`} alt={item.name}
                                             className="w-full h-auto object-cover rounded-[2rem] mb-2 md:rounded-[4rem]" draggable="false"/>
                                    ) : (
                                    ...
                                    item.poster_path ? (
                                        <img src={`${TMDB_IMAGE_BASE_URL}${item.poster_path}`} alt={item.title || item.name}
                                             className="w-full h-auto object-cover rounded-[1rem] mb-[.75rem] lg:rounded-[1.25rem]" draggable="false"/>
                                    ) : (
                                        <div className="w-full h-[8.5rem] bg-slate-800 rounded-[1rem] flex items-center justify-center mb-2 flex-grow">
                                            <img src={movie} alt="Movie Icon" className="w-16 h-16 p-2"/>
                                        </div>
                                    )
                                )}
                                <h3 className="text-sm font-[650] text-center mb-[.25rem] sm:text-base 2xl:text-lg" style={LINE_CLAMP3}>
                                ...
                                {item.vote_average > 0 && item.media_type !== "person" && (
                                    <div className="flex items-center gap-1 mt-[.5rem]">
                                        ...
                                        <span className="text-xs text-white font-[550] md:text-sm 2xl:text-base">
