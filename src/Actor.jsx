...
function Actor() {
    ...
    useEffect(() => {
        const fetchActorData = async () => {
            ...
            try {
                const [actorDetailsRes, actorImagesRes, movieCreditsRes, tvCreditsRes] = await Promise.all([
                    fetch(`${TMDB_BASE_URL}/person/${actorId}?api_key=${TMDB_API_KEY}`),
                    fetch(`${TMDB_BASE_URL}/person/${actorId}/images?api_key=${TMDB_API_KEY}`),
                ...
                if (!movieCreditsRes.ok) {
                    throw new Error(`Failed to fetch filmography: ${movieCreditsRes.statusText}`);
                }
                const movieCreditsData = await movieCreditsRes.json();
                
                if (!tvCreditsRes.ok) {
                    throw new Error(`Failed to fetch TV credits: ${tvCreditsRes.statusText}`);
                }
                const tvCreditsData = await tvCreditsRes.json();

                let combinedCredits = [
                    ...movieCreditsData.cast.map(item => ({...item, media_type: "movie"})),
                    ...tvCreditsData.cast.map(item => ({...item, media_type: "tv"}))
                ];

                const seenIds = new Set();
                const uniqueCredits = combinedCredits.filter(item => {
                    const isDuplicate = seenIds.has(item.id);
                    seenIds.add(item.id);
                    return !isDuplicate;
                });

                const sortedFilmography = uniqueCredits
        ...
        <div>
            ...
            <div className="w-full px-[2rem] flex flex-col items-start justify-center md:px-[3.4rem]">
                <div className="flex items-center gap-5 md:gap-7 xl:gap-10">
                    <div>
                        ...
                        <div>
                        <h1 className="text-white font-[650] text-lg sm:text-xl
                        md:text-2xl">{actor.name}</h1>
                        <p className="text-slate-500 font-[550] text-sm sm:text-base
                        md:text-lg">Actor</p>
                    </div>
                </div>
            </div>

            <div className="w-full px-[2rem] mt-[1.25rem] flex flex-col items-center
            justify-center gap-5 md:px-[3.4rem] lg:flex-row">
                <div className="bg-slate-900 w-full h-[18rem] rounded-[1.25rem] flex
                overflow-hidden sm:h-[24rem] md:h-[30rem] lg:h-[21.2rem] lg:w-[34rem]
                xl:h-[25.3rem]">
                    {/* Image 1 */}
                    <div className="w-1/2 h-full pr-1 py-1 pl-1 flex items-center
                    justify-center">
                        {actorImages[0] ? (
                            ...
                            justify-center rounded-l-[1.25rem]">
                                ...
                                md:w-[5rem] md:h-[5rem] opacity-50" draggable="false"/>
                            </div>
                        )}
                    </div>
                    <div className="w-1/2 h-full flex flex-col pl-1 py-1 pr-1 gap-1">
                        {/* Image 2 */}
                        <div className="w-full h-1/2 pb-0.5 flex items-center
                        justify-center">
                            {actorImages[1] ? (
                                ...
                                items-center justify-center rounded-tr-[1.25rem]">
                                    ...
                                    sm:h-[4.5rem] md:w-[5rem] md:h-[5rem] opacity-50"
                                    draggable="false"/>
                                </div>
                            )}
                        </div>
                        {/* Image 3 */}
                        <div className="w-full h-1/2 pb-0.20 flex items-center justify-center">
                            {actorImages[2] ? (
                                ...
                                <div className="w-full h-full bg-slate-800 flex
                                items-center justify-center rounded-br-[1.25rem]">
                                    <img src={slateActor} alt="Actor Icon"
                                    className="w-[4rem] h-[4rem]
                                    sm:w-[4.5rem] sm:h-[4.5rem] md:w-[5rem] md:h-[5rem]
                ...
                <div className="flex flex-col gap-5 w-full sm:flex-row lg:flex-col lg:w-auto">
                    <div className="bg-slate-900 w-full h-[6rem] rounded-[1.25rem] flex
                    flex-col items-start justify-center gap-1 px-[1.5rem] md:h-[8rem]
                    lg:h-[10rem] lg:w-[17rem] lg:gap-4 xl:h-[12rem] xl:w-[20rem]">
                        <p className="text-slate-400 text-base font-[650] md:text-lg
                        lg:ml-[.25rem] xl:text-xl">Age</p>
                        <h1 className="text-white text-xl font-[550] md:text-2xl
                        lg:ml-[.25rem] lg:text-3xl">{calculateAge(actor.birthday)}</h1>
                    </div>
                    <div className="bg-slate-900 w-full h-[6rem] rounded-[1.25rem] flex
                    flex-col items-start justify-center gap-1 px-[1.5rem] md:h-[8rem]
                    lg:h-[10rem] lg:w-[17rem] lg:gap-2 xl:h-[12rem] xl:w-[20rem]">
                        <p className="text-slate-400 text-base font-[650] md:text-lg
                        lg:ml-[.25rem] xl:text-xl">Overview</p>
                        {actor.birthday && (
                            <div className="flex gap-2 items-center lg:ml-[.25rem]
                            lg:gap-3 lg:mt-[.5rem] xl:mt-[1rem]">
                                <img className="w-[1rem] h-[1rem] lg:w-[1.5rem]
                                lg:h-[1.5rem] xl:w-[2rem] xl:h-[2rem]" src={cake}
                                alt="Birthday Cake Icon" draggable="false"/>
                                <p className="text-white text-xs font-[550] md:text-sm
                                xl:text-base">{formatDate(actor.birthday)}</p>
                            </div>
                        )}
                        {actor.place_of_birth && (
                            ...
                            lg:gap-3">
                                <img className="w-[1rem] h-[1rem] lg:w-[1.5rem]
                                lg:h-[1.5rem] xl:w-[2rem] xl:h-[2rem]" src={location}
                                alt="Location Icon" draggable="false"/>
                                <p className="text-white text-xs font-[550] md:text-sm
                                xl:text-base">{actor.place_of_birth}</p>
                            </div>
                        )}
                    </div>
                </div>
            ...
            {actor.biography && (
                <div className="mx-[2rem] mt-[2rem] md:mx-[3.4rem]">
                    <h1 className="text-white text-lg font-[650] mb-2 md:text-xl
                    xl:text-2xl">Biography</h1>
                    <p className="text-slate-400 text-sm leading-relaxed md:mt-[1rem]
                    md:text-base xl:mt-[1.5rem] xl:text-lg">{actor.biography}</p>
                </div>
            )}

            {filmography.length > 0 && (
                <div className="ml-[2rem] mt-[2rem] md:ml-[3.4rem] md:mt-[3rem]
                xl:mt-[4rem]">
                    <h1 className="text-white text-lg font-[650] mb-[1rem] md:text-xl
                    lg:text-2xl lg:mb-[1.5rem]">Filmography</h1>
                        ...
                        {filmography.map((item) => (
                            ...
                            onClick={() => navigate(`/${item.media_type}/${item.id}`)}>
                                ...
                                xl:h-[12rem] 2xl:w-[9rem] 2xl:h-[13rem]">
                                    {item.poster_path ? (
                                        <img src={`${TMDB_MOVIE_POSTER_BASE_URL}${item.poster_path}`}
